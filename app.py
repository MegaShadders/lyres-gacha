...
def pull():
    ...
    with sqlite3.connect("lyres.db") as con:
        ...
        pity = cur.execute("SELECT id, rarity, count, maximum FROM pity INNER JOIN user_pity ON pity.id = user_pity.pity_id WHERE id IN (SELECT pity_id FROM banner_pity WHERE banner_id = ?) AND user_id = ?", [request.form.get("bannerID"), 3]).fetchall()
        ...
        for k in range(len(pity)):
            cur.execute("UPDATE user_pity SET count = ? WHERE pity_id = ? AND user_id = ?", [counts[k], pity[k][0], 3])   
    ...
    with sqlite3.connect("lyres.db") as con:
        ...
        for i in range(len(units)):
            if unitList[i] in collectedList:
                ...
                units[i] = units[i] + (True, ) # Set to False when not testing
